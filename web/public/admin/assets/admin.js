(function () {
  ...
  function initAdminFormAutoSave() {
    ...
    form.addEventListener('change', function (evt) {
      ...
      if (t && (t.name === 'mosaic_max_columns' || t.name === 'cover_style' || t.name === 'cover_layout'
        || t.name === 'intro_text_layout' || t.name === 'intro_text_layouts' || t.name === 'intro_title_layout_width')) {
        runAdminAutoSave();
        return;
      }
  ...
  if (lightbox) {
    ...
    document.addEventListener('click', function (evt) {
      ...
      if (btn) {
        ...
        evt.preventDefault();
        openLightbox(btn.getAttribute('data-preview'), null);
        return;
      }
  ...
  function adminStartDimsBackfillApi(force) {
    var fd = new FormData();
    fd.set('dims_backfill_start_api', '1');
    if (force) {
    ...
    return fetch(window.location.pathname + window.location.search, {
      ...
      return res.json().then(function (data) {
        if (!res.ok || !data || !data.ok) {
          throw new Error((data && data.error) || 'Neizdevās sākt izmēru ievākšanu');
        }
        return data;
      });
    });
  ...
  function initAdminFieldsetCollapse() {
    function fieldsetStorageKey(form) {
      ...
      if (path.indexOf('settings.php') !== -1) {
        return 'efpic_admin_fieldset_collapsed_settings';
      }
      if (path.indexOf('delivery_new.php') !== -1) {
        return 'efpic_admin_fieldset_collapsed_new';
      }
      return 'efpic_admin_fieldset_collapsed_admin';
    }
