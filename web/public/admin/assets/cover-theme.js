(function () {
  ...
  var BYLINE_SIZES = {
    sm: '0.65rem',
    md: 'clamp(0.75rem, min(2vw, 28.8px), 0.95rem)',
    lg: '1.05rem',
  };

  var TITLE_SIZES = {
    mood: { sm: 'clamp(1.15rem, min(3vw, 43.2px), 1.55rem)', md: 'clamp(1.65rem, min(4.8vw, 69.12px), 2.5rem)', lg: 'clamp(2.35rem, min(6.5vw, 93.6px), 3.75rem)' },
    standard: { sm: 'clamp(1.25rem, min(3.5vw, 50.4px), 1.75rem)', md: 'clamp(1.85rem, min(5.5vw, 79.2px), 3.15rem)', lg: 'clamp(2.6rem, min(7.5vw, 108px), 4.25rem)' },
    split: { sm: 'clamp(1.25rem, min(3.5vw, 50.4px), 1.75rem)', md: 'clamp(1.85rem, min(5.5vw, 79.2px), 3.15rem)', lg: 'clamp(2.6rem, min(7.5vw, 108px), 4.25rem)' },
  };
    ...
    layer.querySelectorAll('.intro-text-free').forEach(function (el) {
      ...
      el.addEventListener('pointerdown', function (evt) {
        ...
        function up(ev) {
          ...
          resolveIntroTextOverlapsInDoc(doc);
        }
    ...
    if (handle && handle.dataset.resizeBound !== '1') {
      ...
      handle.addEventListener('pointerdown', function (evt) {
        ...
        function move(ev) {
          ...
          titleEl.style.setProperty('--intro-title-box-width', String(nw));
        }
  ...
  function updateDeviceScale(deviceEl) {
    ...
    var fillWidth = deviceEl.getAttribute('data-fit') === 'width';

    var scaleW = boxW / designW;
    var scaleH = boxH / designH;
    var scale = fillWidth ? scaleW : Math.min(scaleW, scaleH);
  ...
  function refreshAllPreviews(root, state, fontMap, groupMap, assets, layoutHandlers) {
    ...
    withPreservedFocus(function () {
      ...
      root.querySelectorAll('.admin-cover-live-device').forEach(function (deviceEl) {
        var iframe = deviceEl.querySelector('.admin-cover-live-device__iframe');
        if (!iframe) return;
        ...
        iframe.onload = function () {
          ...
          try {
            var previewDoc = iframe.contentDocument || iframe.contentWindow.document;
            resolveIntroTextOverlapsInDoc(previewDoc);
