(function () {
  ...
  function initFaceSearch() {
    ...
    if (window.EFPIC_FACE_SEARCH_MODE === 'failiem') {
      initFacePersonSearch();
      return;
    }
    if (!window.EFPIC_FACE_SEARCH_URL) return;
    var modal = document.getElementById('faceSearchModal');
    var openBtns = document.querySelectorAll('[data-face-search-open]');
    var closeBtns = document.querySelectorAll('[data-face-search-close]');
    var input = document.getElementById('faceSearchInput');
    var preview = document.getElementById('faceSearchPreview');
    var previewImg = document.getElementById('faceSearchPreviewImg');
    var submitBtn = document.getElementById('faceSearchSubmit');
    var statusEl = document.getElementById('faceSearchStatus');
    ...
    openBtns.forEach(function (btn) {
      btn.addEventListener('click', openFaceModal);
    });
    closeBtns.forEach(function (btn) {
      btn.addEventListener('click', closeFaceModal);
    });
    if (modal) {
      modal.addEventListener('click', function (evt) {
        if (evt.target === modal) closeFaceModal();
      });
    }
    if (input) {
      input.addEventListener('change', function () {
        ...
        if (selectedFile && preview && previewImg) {
    ...
    function applyFaceFilter(tokens) {
      ...
      document.querySelectorAll('.scene-block').forEach(function (scene) {
        var visible = scene.querySelector('[data-token]:not(.face-search-hidden)');
        scene.classList.toggle('face-search-scene-empty', !visible);
      });
      if (banner && bannerText) {
        bannerText.textContent = 'Rāda ' + tokens.length + ' bildes ar tavu seju';
        banner.hidden = tokens.length === 0;
      }
      closeFaceModal();
      if (banner && tokens.length > 0) {
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', clearFaceFilter);
    }
    function pollSearch(pollUrl, pollMs) {
      return fetch(pollUrl, { credentials: 'same-origin', headers: { Accept: 'application/json' } })
        .then(function (res) {
          return res.json();
        })
        .then(function (data) {
          ...
          if (data.status === 'failed') throw new Error(data.error || 'Meklēšana neizdevās');
          return new Promise(function (resolve, reject) {
            setTimeout(function () {
              pollSearch(pollUrl, pollMs).then(resolve).catch(reject);
            }, pollMs || 800);
          });
        });
    }
      ...
      submitBtn.addEventListener('click', function () {
        ...
        fetch(window.EFPIC_FACE_SEARCH_URL, {
          ...
          .then(function (res) {
            return res.json().then(function (data) {
              if (!res.ok || !data || !data.ok) {
                throw new Error((data && data.message) || (data && data.error) || 'Neizdevās sākt meklēšanu');
              }
              return data;
            });
          })
          .then(function (data) {
            if (statusEl) statusEl.textContent = 'Salīdzinu sejas…';
            return pollSearch(data.poll_url, data.poll_ms || 800);
          })
          .then(function (data) {
            var tokens = data.tokens || [];
            if (tokens.length === 0) {
              if (statusEl) statusEl.textContent = 'Nav atrastu bildes — mēģini citu selfiju.';
              return;
            }
          ...
          .catch(function (err) {
            if (statusEl) {
              statusEl.textContent = (err && err.message) || 'Kļūda';
            }
          })
          .finally(function () {
  ...
  function initFacePersonSearch() {
    ...
    function applyFaceFilter(tokens) {
      ...
      document.querySelectorAll('[data-token]').forEach(function (el) {
        var tok = el.getAttribute('data-token') || '';
        if (tok === '') return;
        el.classList.toggle('face-search-hidden', !set[tok]);
      });
      document.querySelectorAll('.scene-block').forEach(function (scene) {
        var visible = scene.querySelector('[data-token]:not(.face-search-hidden)');
        scene.classList.toggle('face-search-scene-empty', !visible);
