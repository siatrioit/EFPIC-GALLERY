...
function efpic_client_collection_download_modal(array $meta, array $ctx, int $collectionCount): string
{
    ...
    $html .= '<p class="muted visitor-dl-hint" id="collectionDownloadModalHint">ZIP arhīvu sagatavosim un nosūtīsim uz tavu e-pastu.</p>';
...
function efpic_client_html(
    ...
    echo $body;
    $scriptVars = array_merge(['EFPIC_CSRF_TOKEN' => efpic_csrf_token()], $extraScriptVars);
...
function efpic_client_render_cover_split_text(array $config, string $name, string $date): string
{
    ...
    $html .= '<p class="gallery-intro-byline">' . efpic_client_esc(efpic_client_gallery_byline_display($config)) . '</p>';
    if ($date !== '') {
        $html .= '<p class="gallery-intro-date gallery-intro-date--split">' . efpic_client_esc($date) . '</p>';
    }
    $html .= '</div>';
...
function efpic_client_render_image_grid_actions(array $gridCtx, array $img): string
{
    ...
    if (!empty($gridCtx['collection_enabled'])) {
        $html .= '<button type="button" class="grid-collection-btn' . ($inCollection ? ' is-selected' : '') . '" data-collection-toggle data-image-token="'
            . efpic_client_esc($tok) . '" aria-label="Atzīmēt" aria-pressed="' . ($inCollection ? 'true' : 'false') . '">';
...
function efpic_client_render_collection_tray(
    ...
    unset($galleryUrl, $visitorStatus);
    $canColZip = efpic_can_download_collection_zip($meta, $ctx, 'web')
        || efpic_can_download_collection_zip($meta, $ctx, 'full');
    $html = '<aside class="collection-tray" id="collectionTray" hidden aria-live="polite">';
    $html .= '<p class="collection-tray-text">';
    $html .= '<span class="collection-tray-label" id="collectionTrayLabel" hidden></span>';
    $html .= '<strong id="collectionTrayCount">0</strong> bildes izvēlētas</p>';
    $html .= '<div class="collection-tray-actions">';
    $html .= '<button type="button" class="btn" id="visitorManageBtn" data-visitor-manage-open hidden>Pārvaldīt</button>';
    $html .= '<button type="button" class="btn" id="selectionSaveCollectionBtn" data-selection-save-collection hidden>Saglabāt izlasē</button>';
    if ($canColZip) {
        $html .= '<button type="button" class="btn primary" id="collectionDlBtn" data-collection-dl-open hidden>Lejupielādēt</button>';
    }
    $html .= '</div></aside>';
...
function efpic_handle_client_gallery(array $config, string $galleryToken, string $method): void
{
    ...
    if ($method === 'POST' && isset($_POST['gallery_password'])) {
        if (!efpic_csrf_verify((string) ($_POST['csrf_token'] ?? ''))) {
            ...
            $body .= '<p class="feed-empty err">Sesijas derīgums beidzies — atjauno lapu un mēģini vēlreiz.</p></div></main>';
    ...
    if (efpic_viewer_context_access_denied($ctx)) {
        efpic_client_html(
            $name,
            '<p class="feed-empty err">Kopīgojamā saite nav derīga.</p>',
            $config,
            'page-auth',
            '',
            [],
    ...
    // Atlases atzīmes un tray — tikai klienta pusē (lokālā atlase / Izlase režīms).
    $gridCtx['collection'] = [];
    $collectionCount = 0;
...
function efpic_client_stream_failiem_image_zip(
    ...
    return efpic_failiem_stream_selected_zip($config, $folderHash, $hashes, $filename);
}
...
function efpic_client_subset_zip_prepare_response(
    array $config,
    array $found,
    array $meta,
    array $ctx,
    ...
    $sceneId = $scope === 'scene' ? (string) ($found['scene_id'] ?? '') : '';
    $filename = efpic_client_zip_filename($slug, $size, $scope, $sceneId);
    if ($images === []) {
...
function efpic_client_zip_prepare_response(
    ...
    if ($scope === 'collection') {
        efpic_client_subset_zip_prepare_response(
            ...
            $size,
            $scope,
            efpic_client_collection_images($meta, $ctx, $galleryToken),
            $galleryToken,
            'atlasītajām bildēm',
        );
    }
...
{
    $tokens = efpic_client_collection_tokens($galleryToken);
    if ($tokens === []) {
        return [];
    }
    $byToken = [];
...
function efpic_handle_client_subset_zip(
    ...
    string $galleryToken,
    string $scope,
    callable $imagesResolver,
    callable $canDownload,
    string $stashScope,
    string $sceneId = '',
): void {
    @set_time_limit(0);
    ...
    if (!$canDownload($meta, $ctx, $size)) {
        http_response_code(403);
        exit;
    }

    $images = $imagesResolver($meta, $ctx, $galleryToken);
    if ($images === []) {
        http_response_code(404);
        exit;
    }
...
function efpic_handle_client_collection_zip(array $config, string $galleryToken): void
{
    efpic_handle_client_subset_zip(
        ...
        $galleryToken,
        'collection',
        static fn (array $meta, array $ctx, string $galleryToken): array => efpic_client_collection_images($meta, $ctx, $galleryToken),
