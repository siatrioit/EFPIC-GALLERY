const fs = require('fs');
const cssPath = 'D:/Dev/projects/EFPIC-GALLERY/web/public/client/assets/client.css';
let t = fs.readFileSync(cssPath, 'utf8').replace(/\r\n/g, '\n');

if (t.includes('gallery-intro--mood')) {
  console.log('blob css already present');
  process.exit(0);
}
if (!t.includes('.theme-efpic-mood')) {
  console.error('missing efpic mood theme rules');
  process.exit(1);
}
...
@keyframes moodIntroBlobMorph {
  0% { border-radius: 58% 42% 52% 48% / 48% 58% 42% 52%; }
  25% { border-radius: 48% 52% 58% 42% / 58% 42% 52% 48%; }
  50% { border-radius: 52% 48% 42% 58% / 42% 58% 48% 52%; }
  75% { border-radius: 42% 58% 48% 52% / 52% 48% 58% 42%; }
...
@media (prefers-reduced-motion: reduce) {
  .gallery-intro-blob { animation: none; }
}

@media (min-width: 768px) {
  .gallery-intro--mood { gap: clamp(32px, 5vh, 56px); }
  .gallery-intro-blob-wrap { width: min(52vw, 480px); }
}
`;

const marker = '\n.topbar-floating {';
if (!t.includes(marker)) {
  console.error('topbar marker not found');
  process.exit(1);
}
t = t.replace(marker, '\n' + blobBlock + marker);

const mediaOld = `  .gallery-intro-title {
...
const mediaNew = `  .gallery-intro-title {
    max-width: min(88%, 56rem);
  }
...
if (!t.includes(mediaOld)) {
  console.error('media marker not found');
  process.exit(1);
}
t = t.replace(mediaOld, mediaNew);

fs.writeFileSync(cssPath, t.replace(/\n/g, '\r\n'), 'utf8');
fs.copyFileSync(cssPath, 'g:/Mans disks/EFPIC-LIVE/web/public/client/assets/client.css');
console.log('mood blob css patched, themes preserved');
